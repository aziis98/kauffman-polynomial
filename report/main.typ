...
The Kauffman polynomial $kL$ is a two-variable polynomial invariant of regular isotopy for unoriented knots and links in 3-dimensional space @Kauffman1990-qe. It is defined using _Skein relations_, more precisely an implicit functional equation.
...
- #strong(link("https://knotinfo.math.indiana.edu/descriptions/gauss_notation.html")[Signed Gauss codes, SG codes]) @knotinfo-gauss-notation -- This is an improvement over the previous representation, we also store the handedness of the crossing. This is the representation we are going to use for the various operations on the knots.

- #strong(link("https://knotinfo.math.indiana.edu/descriptions/dt_notation.html")[Dowker-Thistlethwaite codes, DT codes]) @knotinfo-dt-notation -- This is another, more compact, representation present in KnotInfo and is based on crossing labels. We label each crossing (twice) in order from $1$ to $2n$. Each crossing will get an even and odd label and we take only the even labels in order. When an even number is in under-strand position we mark it with a minus sign.
...
The main source for the section is the article on #link("https://knotinfo.math.indiana.edu/descriptions/pd_notation.html")[PD notation from KnotInfo] @knotinfo-pd-notation. The PD code for a link is generated by labelling each arc of the link with a number. Then we choose a starting point for each component and process each component in order.
...
This is solved by *Signed Gauss Codes* @knotinfo-gauss-notation where we also store the information about the handedness of a crossing.
...
We tried two approaches for our algorithm, the first based on the closed form algorithm present in Kauffman's paper @Kauffman1990-qe and another "naive one" based directly on applying the skein relation. Let's first recap the main formal closed form algorithm for computing the Kauffman polynomial.
...
The main achievement of this project was to check all (normalized) Kauffman polynomials present in the _KnotInfo database_ @knotinfo. Using the python package `database_knotinfo` we loaded all the data and wrote a script called `./check_knotinfo.py` with the following options
